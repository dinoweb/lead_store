...
final class Loader {
	...
	public function library($library) {
		$file = DIR_LOCAL_SYSTEM . 'library/' . $library . '.php';
		if (file_exists($file)) {
			include_once($file);
		}
		else
	...
	public function helper($helper) {
		$file = DIR_LOCAL_SYSTEM . 'helper/' . $helper . '.php';
		if (file_exists($file)) {
			include_once($file);
		}
		else
	...
	public function model($model) {
		$file  = DIR_LOCAL_APPLICATION . 'model/' . $model . '.php';
	...
	public function database($driver, $hostname, $username, $password, $database, $prefix = NULL, $charset = 'UTF8') {
		$file  = DIR_LOCAL_SYSTEM . 'database/' . $driver . '.php';
